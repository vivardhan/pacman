...
//
//#define iOS_ATLEAST_7 SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"7.0")
...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if (self) {
        _camera = -1;
        _qualityPreset = AVCaptureSessionPreset640x480; // AVCaptureSessionPresetMedium
...
- (void)setShowDebugInfo:(BOOL)showDebugInfo
{
    ...
    if (showDebugInfo && !_fpsLabel) {
        ...
        _fpsLabel = [[NSTextField alloc] initWithFrame:frame];
////        _fpsLabel.textColor = [CIColor whiteColor];
//        _fpsLabel.backgroundColor = [CIColor colorWithRed:0.0f Green: 0.0f Blue: 0.0f alpha:0.5f];
       ...
       fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (format == kCVPixelFormatType_420YpCbCr8BiPlanarFullRange) {
        ...
        // TODO pass along U and V image as well.
...
//    AVCaptureDeviceInput *input = [[AVCaptureDeviceInput alloc] initWithDevice:_captureDevice error:&error];
    ...
    NSDictionary *pixelBufferOptions = [NSDictionary dictionaryWithObjectsAndKeys:
                                        ...
                                        nil];
    [_videoOutput setVideoSettings:pixelBufferOptions];
...
- (void)updateDebugInfo {
    if (_fpsLabel) {
        [_fpsLabel setStringValue:[NSString stringWithFormat:@"FPS: %0.1f", _fps]];
